...
fn main() {
    let (n, q) = (1, Z::from(2).pow(24).unwrap());
    ...
    let mut file_a = File::create("parity_check_matrix").unwrap();
    let mut file_r = File::create("trapdoor").unwrap();
    let mut file_conv = File::create("convolution_matrix").unwrap();
